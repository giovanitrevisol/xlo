...
class LoginBloc with LoginValidator {
  final BehaviorSubject<LoginBlocState> _stateContrller =
...
//
  Stream<LoginBlocState> get outState => _stateContrller.stream;
  ...
  Future<bool> loginWithEmail() async {
    _stateContrller.add(LoginBlocState(LoginState.LOADING));

    await Future.delayed(Duration(seconds: 3));

    _stateContrller.add(LoginBlocState(LoginState.DONE));
    return true;
  }

  Future<bool> loginWithFacebook() async {
    _stateContrller.add(LoginBlocState(LoginState.LOADING_FACE));
